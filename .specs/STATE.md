...
- **Feature**: feat-viewer-chat (ADR-051 Accepted, **Implementada** em `feat/viewer-chat`) — Chat LLM nativo no viewer Electron. **Phase 1 MVP shipped**: 14 commits mergeados, 8 IPC channels (4 invoke + 4 streaming events), Zustand store, IndexedDB persistence via idb-keyval, Vercel AI SDK streaming (Anthropic default + OpenAI opcional), RAG opcional via `mem search` subprocess no main, API keys seguras em `app.getPath('userData')/chat-config.json`, settings modal com provider/model/apiKey/useMemory. Componentes: chat-view (orquestrador) + chat-sidebar + chat-thread + chat-message (MarkdownLite regex) + chat-input (Enter envia, Shift+Enter quebra) + chat-settings-modal. Quality gate verde: `gofmt -l .` clean, `go build ./...` exit 0, `tsc --noEmit` exit 0, vitest 1/1, `electron-forge package` exit 0. **Deferral (Plano B)**: Tiptap rich-text, tool calling, sync entre devices. Branch `feat/viewer-chat` aguarda PR target `develop`.
