...
- **Feature**: feat-viewer-chat (ADR-051 Proposed) — Chat LLM nativo no viewer Electron. Spec + ADR + tasks escritas, branch `feat/viewer-chat` aberta a partir de `develop`. **19 tasks** aguardando Execute (Phase 0 bootstrap → Phase 5 docs/PR). Phase 1 MVP scope: Anthropic + OpenAI providers via Vercel AI SDK, streaming via IPC `mem:chat:delta`, persistência IndexedDB (idb-keyval), RAG opcional via `mem search`, API keys seguras no main process, multi-conversa. ~3-4h de trabalho focado. **Estado**: spec.md + tasks.md + adr/051 mergeados em branch; nenhuma implementação ainda — aguarda Execute.
