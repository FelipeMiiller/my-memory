...
7. **🖥 Viewer Desktop (Electron + React + Vite + shadcn/ui + i18next PT/EN) — Phase 1 MVP:** Desktop app cross-platform com Cytoscape bundled (zero-CDN), CSP strict em `viewer/index.html`, locale switcher `pt-BR`/`en-US` persistido em `localStorage`, tabs Graf (active) + Code/Staleness (Phase 2). Decisão arquitetural em [ADR-048](docs/adr/048-viewer-electron-react-vite-shadcn.md) — veja [docs/VIEWER.md](docs/VIEWER.md) para setup.
