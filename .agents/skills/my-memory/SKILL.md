...
> **Esta é a skill canônica de uso do My-Memory.** Qualquer agente (humano ou IA) que precise instalar, configurar, usar ou documentar algo neste projeto deve seguir este guia. Para regras estritas de formatação de **qualquer tipo de memória** (Markdown + JSON Canvas), consulte a skill [`my-memory-format`](.agents/skills/my-memory-format/SKILL.md) — ela é a referência canônica de markup.
...
> [!important]
> **Esta seção é a parte mais importante para quem cria `.md`.** Regras detalhadas de markup vivem na skill [`my-memory-format`](.agents/skills/my-memory-format/SKILL.md) — esta é só a visão geral.
...
### Skills irmãs
- [`my-memory-format`](.agents/skills/my-memory-format/SKILL.md) — **canônica de formatação de qualquer memória do MyMemory** (Markdown Obsidian Flavored + JSON Canvas 1.0)
