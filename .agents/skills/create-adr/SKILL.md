...
> [!important]
> **Canonical markdown reference**: este skill gera arquivos `.md`. Para formatação completa (frontmatter, wikilinks, callouts, anti-patterns, validação pós-criação), consulte a skill canônica [[my-memory-format]]. Em caso de conflito, a skill `my-memory-format` prevalece para artefatos do vault; para ADRs, mantém-se a convenção histórica de links markdown relativos.
